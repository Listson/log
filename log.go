...
func buildRollingLog(logCfg *lumberjack.Logger, cfg *zap.Config, opts ...zap.Option) (*zap.Logger, error) {
	...
	encConstructor, ok := _encoderNameToConstructor[cfg.Encoding]
	if !ok {
		return nil, fmt.Errorf("no encoder registered for name %q", cfg.Encoding)
	}

	enc, err := encConstructor(cfg.EncoderConfig)
	if err != nil {
		return nil, err
	}

	core := zapcore.NewCore(enc, zapcore.NewMultiWriteSyncer(
		zapcore.AddSync(os.Stdout),
		zapcore.AddSync(hook)), cfg.Level)
...
// New initializes logger by opts which can custmoized by command arguments.
func New(opts *Options) *zapLogger {
	...
	var err error
	var l *zap.Logger
	options := []zap.Option{zap.AddStacktrace(zapcore.PanicLevel), zap.AddCallerSkip(1)}
	if opts.Rolling {
		l, err = buildRollingLog(opts.RollingLog, loggerConfig, options...)
	} else {
		l, err = loggerConfig.Build(options...)
	}
	if err != nil {
		panic(err)
	}
